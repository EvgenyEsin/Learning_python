...
__all__ = ['_read_line_or_begin', 'convert_lines']
...
def _read_line_or_begin(fd: TextIO) -> str:
    text = fd.readline()
    if text == '':
        fd.seek(0)
        text = fd.readline()
...
def convert_lines(names: str | Path, numbers: str | Path, results: str | Path) -> None:
    with (
        ...
        open('numbers.txt', 'r', encoding='utf8') as f_numbers,
        ...
        for _ in range(max_len):
            name = _read_line_or_begin(f_names)
            nums_str = _read_line_or_begin(f_numbers)
